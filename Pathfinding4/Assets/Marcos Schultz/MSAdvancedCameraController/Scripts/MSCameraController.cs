...
public class CameraSetting {
	...
	public SettingsFlyCamera FlyCamera_OnlyWindows;
}
[Serializable]
	...
	void Awake(){
		...
		for (int x = 0; x < cameras.Length; x++) {
			if (cameras [x]._camera) {
				...
				if (cameras [x].rotationType == CameraType.TipoRotac.FlyCamera_OnlyWindows) {
	...
	void ManageCameras(){
		...
		case CameraType.TipoRotac.FirstPerson:
			if (CameraSettings.firstPerson.rotateWhenClick) {
				if (Input.GetKey (CameraSettings.firstPerson.keyToRotate) || _enableMobileInputs) {
					rotacX += _horizontalInputMSACC * CameraSettings.firstPerson.sensibilityX;
					rotacY += _verticalInputMSACC * CameraSettings.firstPerson.sensibilityY;
				}
			} else {
				rotacX += _horizontalInputMSACC * CameraSettings.firstPerson.sensibilityX;
				rotacY += _verticalInputMSACC * CameraSettings.firstPerson.sensibilityY;
		...
		case CameraType.TipoRotac.Orbital:
			...
			if (CameraSettings.orbital.rotateWhenClick) {
				if (Input.GetKey (CameraSettings.orbital.keyToRotate) || _enableMobileInputs) {
					xOrbit [index] += _horizontalInputMSACC * (CameraSettings.orbital.sensibility * distanceFromOrbitalCamera [index]) / (distanceFromOrbitalCamera [index] * 0.5f);
					yOrbit [index] -= _verticalInputMSACC * CameraSettings.orbital.sensibility * (CameraSettings.orbital.speedYAxis * 10.0f);
				}
			} else {
				xOrbit [index] += _horizontalInputMSACC * (CameraSettings.orbital.sensibility * distanceFromOrbitalCamera [index]) / (distanceFromOrbitalCamera [index] * 0.5f);
				yOrbit [index] -= _verticalInputMSACC * CameraSettings.orbital.sensibility * (CameraSettings.orbital.speedYAxis * 10.0f);
		...
		case CameraType.TipoRotac.OrbitalThatFollows:
			...
			case SettingsCameraOrbitalThatFollows.ResetTimeType.Time:
				...
				break;
			case SettingsCameraOrbitalThatFollows.ResetTimeType.Input_OnlyWindows:
		...
		case CameraType.TipoRotac.ETS_StyleCamera:
			if (CameraSettings.ETS_StyleCamera.rotateWhenClick) {
				if (Input.GetKey (CameraSettings.ETS_StyleCamera.keyToRotate) || _enableMobileInputs) {
					rotacXETS += _horizontalInputMSACC * CameraSettings.ETS_StyleCamera.sensibilityX;
					rotacYETS += _verticalInputMSACC * CameraSettings.ETS_StyleCamera.sensibilityY;
				}
			} else {
				rotacXETS += _horizontalInputMSACC * CameraSettings.ETS_StyleCamera.sensibilityX;
				rotacYETS += _verticalInputMSACC * CameraSettings.ETS_StyleCamera.sensibilityY;
			...
			break;
		case CameraType.TipoRotac.FlyCamera_OnlyWindows:
			cameraRotationFly.x += _horizontalInputMSACC * CameraSettings.FlyCamera_OnlyWindows.sensibilityX * 15 * Time.deltaTime;
			cameraRotationFly.y += _verticalInputMSACC * CameraSettings.FlyCamera_OnlyWindows.sensibilityY * 15 * Time.deltaTime;
